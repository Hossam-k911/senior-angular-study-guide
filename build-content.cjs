...
if (sections.length !== 15 || questionCount !== 107) {
  throw new Error(`Content is incomplete: found ${sections.length} sections and ${questionCount} questions; expected 15 and 107. ${sections.map(s => `${s.id}:${s.questions.length}`).join(', ')}`);
}
